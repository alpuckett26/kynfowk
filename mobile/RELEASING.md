...
## Plus subscriptions (M98)

Plus subscriptions on iOS are sold **only via web** at `https://kynfowk.com/upgrade` (Square checkout) at this stage. The iOS app does not present a clickable "Subscribe" button — informational text on the Me tab only, per Apple anti-steering rules.

The server still accepts and validates Apple IAP receipts at `/api/native/iap/apple-receipt` and the App Store Server Notifications V2 webhook at `/api/webhook/apple-app-store` is live. These remain harmless and ready for whenever in-app purchase is re-introduced (queued as M102).

---

## Currently NOT wired on iOS (post-M98)

- **Apple In-App Purchase** — `expo-iap` removed in M98. Plus subs on web only. Re-add as M102 when iOS 26 is more settled.
- **AdMob mobile ads** — `react-native-google-mobile-ads` removed in M98. Re-add as M101 behind a feature flag.
- **App Tracking Transparency** — `expo-tracking-transparency` removed in M98. Only needed when ads come back.
- **Photo / avatar upload** — `expo-image-picker` removed in M98. Re-add as M99 (avatars first) and M100 (full photos route).
- **Family Reel on home tab** — Pulled `expo-image-picker` into the home boot path; removed in M98. Re-introduce after M99 once boot is verified stable.
