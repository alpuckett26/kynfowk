 ...
 *
 * Shared notification-payload types used by the server-side push sender
 * (`lib/send-push.ts`) and any other code that constructs notification
 * payloads delivered to the Expo client.
 *
 * The Capacitor WebView wrapper that previously consumed these types
 * (DeepLinkHandler / PushNotificationHandler) was removed in M52 — we
 * ship a pure web app + a separate React Native (Expo) client.
 */
